...
In this section, theorems for elastic structures are specialized for beam structures. This topic here is treated using Bernoulli-de Saint Venant beam model (or everything is written w.r.t. principal axes), so that actions are decoupled, in order to get ready to face some small problems that can be solved with pen and paper.
