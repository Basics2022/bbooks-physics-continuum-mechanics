...
having exploited the properties of the mixed vector product. If $\mathbf{u}$, $\mathbf{v}$ repsectively represent a displacement and a rotation field $\mathbf{u} = \widetilde{\mathbf{s}}$, $\mathbf{v} = \widetilde{\boldsymbol\theta}$,
